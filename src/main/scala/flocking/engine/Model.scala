...
trait Model {
  ...
  def newBird[B <: Bird](p: Point, h: Heading): B
  def worldState2SensorsState[SensorsState](g: GraphBirds): SensorsState

  def randomBird: Bird = newBird(Position(Random.nextDouble() * worldWidth, Random.nextDouble() * worldHeight),
...
//      this))
  ...
  def updateBirds(graph:GraphBirds): Seq[Bird] = graph.birds.indices.map((b: Int) => graph.birds(b).update(worldState2SensorsState(graph)))
